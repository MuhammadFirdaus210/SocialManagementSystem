...
const VerifikasiKUBE = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const verifikasiDataa = resData.kubes.filter((item) => item._id === id)[0];
        setVerifikasiData(verifikasiDataa);
        console.log("Data Verifikasi KUBE:", verifikasiDataa);
        if (verifikasiDataa) {
          setFormData({
            tahun: verifikasiDataa.verifikasiKUBE.tahun,
            jumlahBantuan: verifikasiDataa.verifikasiKUBE.jumlahBantuan,
            sumberDana: verifikasiDataa.verifikasiKUBE.sumberDana,
            status: 'Approved'
          });
          setIsEdit(true);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const res = await fetch(`http://localhost:4000/api/v1/verifikasi/Kube/update/${verifikasiData.verifikasiKUBE._id}`,{
          method: 'PUT',
          headers: {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-white shadow-lg rounded-lg">
      <h2 className="text-2xl font-bold text-gray-700 mb-6">Verifikasi Data Proposal KUBE</h2>
