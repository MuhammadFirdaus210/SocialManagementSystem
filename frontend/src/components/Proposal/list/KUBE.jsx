...
const TableKUBE = ({ jenis = "ppks", tipe }) => {
  ...
  const columns = useMemo(
    () => [
            ...
            <div className="relative">
              ...
              {isShow && (
                <ul className="rowMenu fixed bg-slate-100/80 border rounded shadow-md w-32 z-[1000] p-1">
                  <a
                    href={`/proposal/kube/detail/${info.row.original._id}`}
                  ...
                  <a
                    href={`/proposal/kube/edit/${info.row.original._id}`}
      ...
      },
      {
        ...
        cell: (info) => (
          <a
            href={`/proposal/kube/verifikasi/${info.row.original._id}`}
      ...
      },
      {
        ...
        cell: (info) => (
          <a
            href={`/proposal/kube/evaluasi/${info.row.original._id}`}
  ...
  const handleDeleteAction = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/kube/delete/${idRow}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: {
